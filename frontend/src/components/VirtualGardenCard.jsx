...
import LegacyCertificateCard from "./LegacyCertificateCard";

const VirtualGardenCard = ({
  impact,
  totalWaterSaved = 0,
  title = "My Virtual Garden",
  showCertificate = true,
}) => {
  const waterSaved = Math.round(Number(totalWaterSaved || impact?.total_water_saved_liters || 0));
  ...
  return (
    <section style={styles.card}>
      <div style={styles.treeWrap}>
        <img
          src={`${API_BASE_URL}/static/trees/${getTreeImageFilename(stage.key, {
            animated: true,
          })}`}
          alt={stage.label}
      ...
      <p style={styles.totalWaterText}>Total saved: {waterSaved.toLocaleString()} L</p>
      ...
      {showCertificate && (
        <LegacyCertificateCard
          certificate={impact?.legacy_certificate}
...
export default VirtualGardenCard;
